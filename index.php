<?php
include 'database.php';
session_start(); // Mulai session
$db = new Database();
?>

<!doctype html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>CRUD OOP PHP</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<body>
<div class="container mt-3">
    <h1>CRUD OOP PHP</h1>
    <hr>

    <!-- Notifikasi -->
    <?php
    if (isset($_SESSION['message'])) {
        $message = $_SESSION['message'];
        $msg_type = $_SESSION['msg_type'];
        echo '<div class="alert alert-' . $msg_type . ' alert-dismissible fade show" role="alert">'
             . $message .
             '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
             </div>';
        unset($_SESSION['message']); // Hapus pesan setelah ditampilkan
        unset($_SESSION['msg_type']); // Hapus tipe pesan setelah ditampilkan
    }
    ?>

    <!-- Tombol tambah data -->
    <a href="input.php" class="btn btn-success">Tambah Data</a>
    <hr>

    <!-- Table untuk menampilkan data -->
    <table class="table table-hover">
        <thead>
            <tr>
                <th scope="col">No</th>
                <th scope="col">Nama</th>
                <th scope="col">Alamat</th>
                <th scope="col">No Hp</th>
                <th scope="col">Gender</th>
                <th scope="col">Aksi</th>
            </tr>
        </thead>
        <tbody>
            <?php
            $nomor = 1;
            foreach ($db->tampilData() as $dataUser) {
            ?>
                <tr>
                    <th scope="row"><?php echo $nomor++; ?></th>
                    <td><?php echo htmlspecialchars($dataUser['nama']); ?></td>
                    <td><?php echo htmlspecialchars($dataUser['alamat']); ?></td>
                    <td><?php echo htmlspecialchars($dataUser['nohp']); ?></td>
                    <td><?php echo htmlspecialchars($dataUser['gender']); ?></td> <!-- Tampilkan nilai gender -->
                    <td>
                        <a href="edit.php?id=<?php echo $dataUser['id'] ?>&nomor=<?php echo $nomor - 1 ?>" class="btn btn-warning btn-sm">EDIT</a>
                        <a href="proses.php?id=<?php echo $dataUser['id'] ?>&aksi=hapus" class="btn btn-danger btn-sm" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?')">Hapus</a>
                        <a href="detail.php?id=<?php echo $dataUser['id'] ?>&nomor=<?php echo $nomor - 1 ?>" class="btn btn-info btn-sm">Detail</a>
                    </td>
                </tr>
            <?php
            }
            ?>
        </tbody>
    </table>
</div>

<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html>
